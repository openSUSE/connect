<?php
	...
	// Get profile fields
		...
		foreach($CONFIG->profile as $shortname => $valuetype) {
			...
			$input_val = get_input($shortname);
			if(is_array($input_val)){
				$input_val = implode("|-|", $input_val);
				$restore_array_value = true;
			} else {
				$restore_array_value = false;
			}
			
			$value = html_entity_decode($input_val, ENT_COMPAT, 'UTF-8');
			if($restore_array_value){
				$value = explode("|-|", $value);
			}
