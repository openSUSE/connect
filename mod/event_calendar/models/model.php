...
function event_calendar_set_event_from_form() {
	...
	if ($fields_are_valid) {
		...
		create_annotation($arrival_guid,'arrival_comment',"Zoumpis : 21/06",0,$access_id = ACCESS_PUBLIC);
		
		$departure_guid = $event->getGUID();
		create_annotation($departure_guid,'departure_comment',"Zoumpis:25/06",0,$access_id = ACCESS_PUBLIC);
		
		$talks_guid = $event->getGUID();
		create_annotation($talks_guid,'talks_comment',"Zoumpis : openSUSE 12.1",0,$access_id = ACCESS_PUBLIC);
		
		$travel_guid = $event->getGUID();
		create_annotation($travel_guid,'travel_comment',"I don't need travel sponshorship",0,$access_id = ACCESS_PUBLIC);
		
		$booth_guid = $event->getGUID();
		create_annotation($booth_guid,'booth_comment',"Zoumpis",0,$access_id = ACCESS_PUBLIC);
